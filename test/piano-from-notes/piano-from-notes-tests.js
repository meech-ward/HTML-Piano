...
function buildPianoWithNotes() {
  return throwableFunction.bind(this).apply(this, [pianoBuilder.buildPianoWithNotes].concat(Array.from(arguments)));
}

describe("piano", function() {
  describe('#buildPianoWithNotes', () => {
    ...
    context("given two valid note data objects", () => {
      ...
      context("given the same valid note", () => {
        it("should store that note data", () => {
          let piano = {};
          let note = noteData("c", 0);
          pianoBuilder.buildPianoWithNotes.call(piano, note, note);
          expect(piano.startNoteData).to.equal(note);
          expect(piano.endNoteData).to.equal(note);
        });
      });
    });
  });
